    ...
    implements vscode.CodeLensProvider<AgentStateLens>
{
    ...
    async create_client() {
        ...
        if (!autostart) {
            ...
            if (await tcpPortUsed.check(serverPort)) {
                vscode.window.showErrorMessage(`port ${serverPort} is being used, try restarting using the 'Rift: Restart' command`, "Restart").then((selection) => {
    ...
    async create(agent_type: string) {
        ...
        const getEditorMetadata = (editor: TextEditor | undefined): EditorMetadata => {
        ...
        const getAgentParams = (editor: TextEditor | undefined): AgentParams => {
            ...
            const visibleEditorMetadata: EditorMetadata[] = vscode.window.visibleTextEditors.map(getEditorMetadata);
