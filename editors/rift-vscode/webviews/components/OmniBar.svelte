<script lang="ts">
    ...
    render: () => {
      return {
        onStart: (props) => {
          ...
          ); // add the enter command to the files
        },
        ...
        onKeyDown(props) {
          ...
          if (props.event.key == "ArrowUp") {
            ...
            focusedFileIndex.update(
              (i) => (i + $filteredFiles.length - 1) % $filteredFiles.length,
            );
          }
  ...
  function sendMessage() {
    ...
    if (
      $state.agents[$state.selectedAgentId].taskWithSubtasks?.task.status ===
        "done" ||
      $state.agents[$state.selectedAgentId].taskWithSubtasks?.task.status ===
        "error"
    ) {
    ...
    if (!editor)
      throw new Error("no editor in sendMEssage() function in OmniBar.svelte");
...
<div
  class="p-2 border-t border-b border-[var(--vscode-input-background)] w-full relative flex flex-row"
>
  <div class="justify-self-end flex mr-1">
    <button
      on:click={() => editor?.chain().focus().insertContent("@").run()}
